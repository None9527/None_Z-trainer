...
class LoRANetwork(nn.Module):
    ...
    def _convert_name_to_key(self, name: str) -> str:
        ...
        while i < len(parts):
            ...
            if part.isdigit():
                result.append(part)
            # to_q, to_k, to_v 保持为整体
            elif part == 'to' and i+1 < len(parts) and parts[i+1] in ['q', 'k', 'v']:
                result.append(f'to_{parts[i+1]}')
                i += 1
            # to_out → to_out.0 (Attention.to_out is nn.ModuleList, Linear at [0])
...
def create_network(
    ...
    **kwargs,
    ...
    if train_norm:
        # 移除 norm 排除规则，允许 norm 层被 LoRA 训练
        exclude_patterns = [p for p in exclude_patterns if "norm" not in p]
