...
:: Define interpreter paths (embedded or system)
...
:: Python module path - v2 uses trainer_core for zimage_trainer
set "PYTHONPATH=%SCRIPT_DIR%backend\trainer_core;%PYTHONPATH%"
...
echo.
echo Options:
echo   --port, -p PORT    Set port (default: 28000)
...
:: Apply defaults
if "%MODEL_PATH%"=="" set "MODEL_PATH=%SCRIPT_DIR%models"
...
:: Start v2 server (from project root, using module path)
cd /d "%SCRIPT_DIR%"
if "%DEV_MODE%"=="1" (
    echo [Dev Mode] Hot reload enabled
    "%PYTHON_EXE%" -m uvicorn backend.interface.main:app --host %TRAINER_HOST% --port %TRAINER_PORT% --reload --reload-dir "%SCRIPT_DIR%backend" --log-level info
) else (
    "%PYTHON_EXE%" -m uvicorn backend.interface.main:app --host %TRAINER_HOST% --port %TRAINER_PORT% --log-level warning
)
